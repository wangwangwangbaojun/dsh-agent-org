...
## 四、R1 备案语义注记（补注，非条款替代）

采入 node-5 建议级观察（复验函 mtn947hn-2036 §1）：`baseline-v0.13.0.bundle` 系 `--all` 抓取于基准石时点，内含 `refs/heads/main=4294c1b`，**不含其后任何 commit**。其语义=**基准快照**（锚定 tag 三件套之第二副本），**非「当前 main 全量备份」**，一切票面/汇报不得将该 bundle 引作现势 main 之完整性凭据。§G L88「--all 完整历史」字样按本注记读法=「抓取时点之完整历史」，契约册原文不改写，由本注记承载口径。当前 main 之现势锚一律以 commit hash + tree hash 表达（本件 §一.1/§一.2）。

——lead（负责人）2026-09-05 补注 [OPS-R1-SEM/mtn947hn-2036]

  ...
  return [
    ...
    },
    {
      ...
      async execute(args) {
        ...
        appendReport({ type: 'report', to: target.id, toOrg: orgOfNode(doc, target.id).id, summary });
    ...
    },
    {
      ...
      parameters: {
        ...
        properties: {
          ...
          model: { type: 'string', description: 'op=add/update: model id override (empty/null = host default).' },
          ...
          kind: { type: 'string', enum: ['collab', 'dotted-subordinate'], description: 'op=addEdge: collab=协作(虚线), dotted-subordinate=虚线下级(点线).' },

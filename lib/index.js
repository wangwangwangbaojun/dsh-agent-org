 ...
 * - systemPrompt 使用指引小节
 */
import { appendFileSync, existsSync, mkdirSync, openSync, readSync, closeSync, readdirSync, readFileSync, statSync, writeFileSync } from 'node:fs';
import { spawnSync, execFile } from 'node:child_process';
import { homedir } from 'node:os';
import { dirname, join } from 'node:path';
import { OrgError, backupOrg, byId, chainOf, LIMITS, loadOrg, mutate, orgOfNode, prepareDoc, renderChart, saveOrg } from './org.js';
...
/** 从头 24KB 文本提取会话摘要行（sessionHead / sessionHeadTrunc 共用；两轨解析同一逻辑）。 */
function extractSessionHead(text) {
  const head = {};
...
/** 会话头摘要（全量同步版）：/session 详情端点专用——语义与响应面零改动。 */
export function sessionHead(file) {
  try {
    const r = spawnSync('zstd', ['-dc', file], { maxBuffer: 64 * 1024 * 1024, timeout: 15000, encoding: 'utf8' });
    return extractSessionHead(r.stdout ?? '');
  } catch { return {}; }
}

// PERF-SESS-1：/sessions 列表端点旧实现 = 120×串行 spawnSync 全量解压，同步阻塞
// web 主事件循环（实测 :3080 内稳定 16s，前端 8s 轮询收不到 → 面板显空）。
// 列表只需要头 24KB，三招收口：①头增量缓存（键 file|mtime|size，会话文件只追加，
// 未变条目零解压——热路径）；②变/新条目并行（并发 8）execFile 异步解压；
// ③输入截断只喂前 32KB（zstd 块流式，premature-end 时头字段已完整解出；
// stdout 空才回退全量）。并行+截断基准 407ms/120（串行 1366ms）。
const HEAD_CACHE_MAX = 400; // 会话池上限 120×若干轮换代；LRU 粗清（超限即整体折半）
const headCache = new Map(); // `${file}|${mtimeMs}|${size}` → head（进程内缓存，重启清零无正确性风险）
function headCacheGet(key) {
  const v = headCache.get(key);
  if (v !== undefined) { headCache.delete(key); headCache.set(key, v); } // touch→队尾
  return v;
}
function headCacheSet(key, v) {
  if (headCache.size >= HEAD_CACHE_MAX) for (const k of [...headCache.keys()].slice(0, HEAD_CACHE_MAX >> 1)) headCache.delete(k);
  headCache.set(key, v);
}

/** 读文件前 n 字节（截断喂 zstd stdin 用）。 */
function readHeadBytes(file, n = 32768) {
  let fd;
  try {
    fd = openSync(file, 'r');
...
function zstdPipe(buf) {
  return new Promise((resolve) => {
    const p = execFile('zstd', ['-dc'], { maxBuffer: 32 * 1024 * 1024, timeout: 8000, encoding: 'utf8' },
      (err, stdout) => resolve(stdout ?? ''));
    p.stdin.end(buf);
  });
}
function zstdFileFull(file) {
  return new Promise((resolve) => {
    execFile('zstd', ['-dc', file], { maxBuffer: 64 * 1024 * 1024, timeout: 15000, encoding: 'utf8' },
      (err, stdout) => resolve(stdout ?? ''));
  });
}

/** 异步会话头（阶梯截断）：64KB 输入 →（无 title 时）256KB → 全量。
 *  实测教训：5.2MB 低熵会话 32K 输入只解出 12KB 文本，session/title 全在盲区内
 *  且 sessionId 在场不会触发回退 → 静默丢 title（QA 复现于本会话本体）。
 *  title 行可晚于首条 user/message（模型产出后才有），故判据必须含 title 缺失升级。
 *  升级档实测开销 ~10ms/会话（5.2MB 文件），最坏面全量回退与旧版语义一致。 */
async function sessionHeadTrunc(entry) {
  for (const kb of [64, 256]) {
    const buf = readHeadBytes(entry.file, kb * 1024);
    if (buf === null) break;
    const head = extractSessionHead(await zstdPipe(buf));
    if (head.sessionId !== undefined && head.title !== undefined) return head;
  }
  return extractSessionHead(await zstdFileFull(entry.file));
}
...
async function mapWithConcurrency(items, width, fn) {
  ...
  const workers = Array.from({ length: Math.min(width, items.length) }, async () => {
    for (let k = (i += 1) - 1; k < items.length; k = (i += 1) - 1) out[k] = await fn(items[k], k);
  });
  await Promise.all(workers);
  return out;
}

/** 列表端点专用：扫描 + 并发/缓存组装头。响应形状与旧版逐字段一致。 */
export async function listRoleSessionsHeaded() {
  const entries = listRoleSessions();
  const stale = [];
  const rows = entries.map((s) => {
    const key = `${s.file}|${s.mtime}|${s.size}`;
    const cached = headCacheGet(key);
    if (cached !== undefined) return { ...{ dir: s.dir, mtime: s.mtime, size: s.size }, ...cached };
    stale.push({ s, key });
    return null; // 占位，稍后回填
  });
  if (stale.length > 0) {
    const filled = await mapWithConcurrency(stale, 8, async (it) => {
      const head = await sessionHeadTrunc(it.s);
      headCacheSet(it.key, head);
      return head;
    });
    let j = 0;
    for (let i = 0; i < rows.length; i += 1) {
      if (rows[i] === null) {
        const s = stale[j].s;
        rows[i] = { dir: s.dir, mtime: s.mtime, size: s.size, ...filled[j] };
        j += 1;
      }
    }
  }
...
function createTools() {
    ...
    },
    {
      ...
      parameters: {
        ...
        properties: {
          ...
          maxTokens: { description: `op=update: integer 1..${LIMITS.maxTokens} or null (= default).` },
      ...
      async execute(args) {
        ...
        if (op === 'update') {
          request.patch = {
            ...
            ...(args?.title !== undefined ? { title: args.title } : {}),
            ...(args?.systemPrompt !== undefined ? { systemPrompt: args.systemPrompt } : {}),
            // V14B-FIX2·S1（评审函 mtncm3m3-hyzg 建议①）：model 形状归一化——string→{model}（schema 演进期
            // 仍可能有调用面喂 string，直接透传会被 sanitizeModel fail-fast 拒/旧版吞 {}），对象原样透传、
            // undefined 零触碰。不改 schema、不改 org.js；与形状终态 A/B 两案均兼容（架构师另裁线）。
            ...(args?.model !== undefined ? { model: typeof args.model === 'string' ? { model: args.model } : args.model } : {}),
            ...(args?.toolScope !== undefined ? { toolScope: args.toolScope } : {}),
            ...(args?.maxTokens !== undefined ? { maxTokens: args.maxTokens } : {}),
          };
        }
  ...
  // BUG-V14B-4（DEF-PATCHDROP-1，node-4 立案函 mtnlc4f0-q9n3）：工具边界未知键点名必拒门。
  // schema 已声明 additionalProperties:false，但已装 harness 运行时不校验工具入参（提示级声明，
  // node-4 实测不拦），拼错/幻觉键可直达 execute；而 op=update 打包层按已知键白名单复制、
  // 其余 op 同理只读白名单字段——未知平铺键在抵达 org.js update 未知键门之前即被吞：
  //   混合形态（合法键+拼错键）＝合法键落盘＋拼错键静默蒸发＋返回成功文案＝BUG-V14 残签；
  //   纯未知键形态退化为泛化「未携带任何已知字段」，失去点名（V14B-3 点名仅覆盖域层 patch 面）。
  // 修法：在工具边界按 schema properties 单一事实源（Object.keys 取 own keys，免疫 in 原型键漏网）
  // 对未知键点名必拒，与域层未知键门同判据同文案形状；门在 execute 入口、loadOrg/saveOrg 之前＝
  // 盘字节级零变化；置于 op 分发之前故覆盖全部 op（add 面拼错 sytemPrompt 同形残面一并收口）。
  // 金样零误伤：Web 面 client.js patchOf 恰发已知键、harness 正常调用不发未知键。
  // 不直接改 execute 函数体：本文件各工具 def 的自述面（name→output 段）由 test/org-toolface-enum-gate
  // 等门禁按源码文本界定提取，schema 上提/搬移会移位其锚定面——故取「数组外裹门」零动自述面形态。
  const mutateDef = defs.find((def) => def.name === 'org_mutate');
  const knownKeys = new Set(Object.keys(mutateDef.parameters.properties));
  const rawExecute = mutateDef.execute;
  mutateDef.execute = async (args) => {
    const unknownKeys = Object.keys(args ?? {}).filter((key) => !knownKeys.has(key));
    if (unknownKeys.length > 0) {
      throw new OrgError(`org_mutate 入参含未知字段：${unknownKeys.join(', ')}（已知字段：${[...knownKeys].join('/')}）`);
    }
  ...
  const route = async (req, res) => {
    ...
    try {
      ...
      if (req.method === 'GET' && suffix === '/sessions') {
        json(res, 200, { ok: true, sessions: await listRoleSessionsHeaded() });
        return;
      }

 ...
 * 本文件零原子重命名系统调用直用、零第二套 tmp→rename 原子写对。
 */
...
export const TEAM_REPLY_SUMMARY_MAX = 300;              // §C4：回投兜底 summary=回投正文前 300 字
...
/**
 * §C4 回投兜底完结（BE-V14-B 增量·纯新增导出；ARCH-RULING-BEV14B-C4 甲案。
 * 既有行零 diff：completeTask 不重构，函数内少量重复是故意的——DRY 让位于边界纪律）。
 * daemon 完成回投落盘后调用：锁内按 dispatchMessageId 等值 ∧ status===running 双项精确命中（§C4 权威连接键，
 * 绝不做正文正则解析）：ok→done+finishedAt+doneSource=reply，非 ok→failed+doneSource=reply（failed 恢复=plan 新 id，禁反转）；
 * 一切不命中（team.json 不存在 / id 不符 / 已终态 / 超时先翻 / 重复回投 / 另一写者抢锁）=返回 'skipped'：
 * 不抛错、rev 零增、级联零触发、盘字节零变化——hook 尽力面，skip ≠ 异常（与 OPS-V15 §8.2 同构）。
 * summary 截断 300 按码点切（防 UTF-16 slice 劈开代理对）；300 < TEAM_SUMMARY_MAX=2000，不叠引用。
 * 级联在锁外 recomputeAndDispatch（与 org_team_done 处理路同形）；failed 下游 blocked 由 derive() 既有面自然导出。
 */
export function completeFromReply(orgPath, dispatchMessageId, { ok = true, summary = '' } = {}) {
  const summaryText = [...String(summary ?? '')].slice(0, TEAM_REPLY_SUMMARY_MAX).join('');
  let hit;
  try {
    hit = withTeamLock(orgPath, () => {
      const doc = loadTeamDoc(orgPath);
      if (doc === null) return 'skipped';
      const t = doc.tasks.find((x) => x.dispatchMessageId === dispatchMessageId && x.status === 'running');
      if (!t) return 'skipped';
    ...
    // 抢锁失败=本轮让位（下轮空闲 tick 兜底）；形状损坏照上抛（写点在匹配之后，天然零写）
    if (err instanceof OrgError && err.code === 'TEAM_LOCK_BUSY') return 'skipped';
    throw err;
  }
  if (hit === 'skipped') return 'skipped';
  recomputeAndDispatch(orgPath);
  return hit;
}

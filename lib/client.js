...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const css = `
      ...
      /* —— SBB 悬浮「滑到最底下」球（docs/FE-SBB-DESIGN-v1.md §3；sticky 末子；实高 28px 防 overflow 裁切半圆，-18px 负边距回收占位≈不增 scrollHeight，A9 精神守恒）—— */
      .dshao-sbb-slot{position:sticky;bottom:10px;height:28px;margin-bottom:-18px;display:flex;align-items:center;justify-content:center;pointer-events:none;z-index:2}
      .dshao-sbb{pointer-events:auto;width:28px;height:28px;border-radius:50%;display:grid;place-items:center;background:var(--dsw-alias-bg-layer-1);border:1px solid var(--dsw-alias-border-l1);color:var(--dsw-alias-label-secondary);cursor:pointer;transition:opacity .15s ease;box-shadow:0 2px 8px rgba(0,0,0,.12)}
      ...
      @media (prefers-reduced-motion:reduce){.dshao-sbb{transition:none}}
    ...
    // ———— SBB 悬浮「滑到最底下」球（team:sbb-fe · docs/FE-SBB-DESIGN-v1.md §2-§3）————
    // S1 可见性纯函数（test/sbb-lock 源文提取 eval 真值表）：溢出(>8px) ∧ 距底>SBB_NEAR。
    // SBB_NEAR=120 单点定义，与 C1 自动跟底阈值（feedScrollRef effect 的 nearBottom<120）同值同向
    //（内容锚对账 A6，两处各自单点禁第三复制）；距底恰 120=隐藏（开区间钉死，A4 案面）。
    const SBB_NEAR = 120;
    const sbbVisible = (el) => el.scrollHeight - el.clientHeight > 8 && el.scrollHeight - el.scrollTop - el.clientHeight > SBB_NEAR;

    // sticky-slot 工厂：挂载=各自 scroller 末子；宿主=closest 最近 SBB 宿主标记（禁 window 级滚动量误判）。
    // S2 三触发=host scroll(passive:true) + 无 dep effect（数据驱动重渲染即重算，防 live-append 失明）+ resize。
    // S3 点击平滑回底（reduced-motion 瞬跳）；smooth 途中滚轮打断不强拉回；S6 真 button，隐藏态 visibility 退 tab 序。
    const SbbSlot = () => {
      const btnRef = React.useRef(null);
      const hostRef = React.useRef(null);
      React.useEffect(() => {
        const host = btnRef.current?.closest('[data-sbb-host]');
        if (host === null || host === undefined) return;
        hostRef.current = host;
        const sync = () => btnRef.current?.setAttribute('data-show', sbbVisible(host) ? '1' : '0');
        host.addEventListener('scroll', sync, { passive: true });
        window.addEventListener('resize', sync);
        sync();
        return () => { host.removeEventListener('scroll', sync); window.removeEventListener('resize', sync); };
      });
      ...
      return h('div', { className: 'dshao-sbb-slot' },
        h('button', { ref: btnRef, className: 'dshao-sbb', type: 'button', 'aria-label': '滑到最底下', 'data-show': '0', onClick: scrollToBottom },
          h('svg', { width: 12, height: 12, viewBox: '0 0 24 24', fill: 'none', stroke: 'currentColor', strokeWidth: 2.4, strokeLinecap: 'round', strokeLinejoin: 'round' },
            h('path', { d: 'M5 9l7 7 7-7' }))));
    };

    // PERF-SESS-1 前端半区：默认 15s 超时（AbortController）——后端已亚秒，
    // 挂起=进程级异常，快速失败进调用方 catch（工作过程轮询会显示 sessionsErr），
    // 不再无限吊住轮询链。慢路由（transcript 详情）显式放宽。
    async function call(suffix, body, timeoutMs = 15000) {
      ...
      try {
        ...
        throw e?.name === 'AbortError' ? new Error(`请求超时（${Math.round(timeoutMs / 1000)}s）`) : e;
    ...
    function OrgSettings() {
      ...
      React.useEffect(() => {
        ...
        const tick = async () => {
          ...
          if (openSession !== null) { try { setTranscript(await call(`/session?dir=${encodeURIComponent(openSession)}`, undefined, 35000)); } catch { /* 静默 */ } }
        };
        void tick();
      ...
      const feedView = h('div', { className: 'dshao-feed', 'data-sbb-host': '1', ref: feedScrollRef },
      ...
      const processView = h('div', { className: 'dshao-feed' },
        ...
        visibleSessions.length === 0
          ...
          : visibleSessions.slice(0, 40).map((s) => h('div', {
              ...
              onClick: () => { setOpenSession(openSession === s.dir ? null : s.dir); if (openSession !== s.dir) { setTranscript(null); void call(`/session?dir=${encodeURIComponent(s.dir)}`, undefined, 35000).then(setTranscript).catch(() => setTranscript({ events: [{ kind: 'text', text: '加载失败' }] })); } },
            },
        ...
        openSession !== null && Array.isArray(transcript?.events)
          ? h('div', { className: 'dshao-proc-scroll', 'data-sbb-host': '1' },
              h('div', { className: 'dshao-panel-head' },

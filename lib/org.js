...
function sanitizeModel(raw) {
  ...
  const fallback = text(model.fallback, LIMITS.provider, 'model.fallback', false);
  ...
  if (op === 'addOrg') {
    ...
    if (op === 'add') {
      ...
      const node = {
        ...
        toolScope: { allow: scopeList(request.toolScope?.allow, 'toolScope.allow'), deny: scopeList(request.toolScope?.deny, 'toolScope.deny') },
        maxTokens: null,
        layout: null,
      ...
      // BUG-V14B-2 必修项（QA-V14 实测 2026-09-05）：patch 未知键整层被无视＝静默假成功，必拒。
      // 票面「零有效键也拒」为可选项，与 QA-GATE1 A4（lead 裁定③冻结钉：空入参=成功前缀+盘零变化）
      // 直接冲突，本票不动他人门禁，故可选项不落地；空 patch 维持等旧 no-op。若未来 A4 换钉，
      // 恢复点＝此处加 `if (patchKeys.length === 0) throw`（金样 test/bugv14b-update-gate.test.mjs 2b 已留反案注释）。
      ...
      if (patch.toolScope !== undefined) {
        const allow = scopeList(patch.toolScope?.allow, 'toolScope.allow');
        const deny = scopeList(patch.toolScope?.deny, 'toolScope.deny');
